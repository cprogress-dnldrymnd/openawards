...
<div id="primary" class="row-fluid">
    <div id="content" role="main" class="span8 offset2">
        ...
        <?= hero('FAQs - '.$current_term_object->name, $current_term_object->description) ?>
        <div class="title-wrapper title-wrapper-v3 pt-0">
            <div class="container text-center">
                <div class="faqs-category-filter">
                    <div class="container">
                        <button class="btn btn-outline-primary d-md-none w-100 collapsed" 
                                type="button" 
                        ...
                        <div class="collapse d-md-block" id="mobileFilterCollapse">
                            <div class="row justify-content-center faqs-filter-holder">
                                <?php foreach ($terms as $term) { 
                                    ...
                                    if ($url_cat) {
                                        $checked = ($term->term_id == $url_cat) ? 'checked' : '';
                                    } else {
                                        $checked = ($term->term_id == $current_term) ? 'checked' : '';
                                    }		
                                ?>
                                <div class="col-auto mb-2">
                                    <?php $term_link = get_term_link($term); ?>
                                    <input id="faqs_category_<?= $term->term_id ?>"  
                                           type="radio"  
                                           name="faqs_category"  
                                           value="<?= $term->term_id ?>"  
                                           data-url="<?= $term_link ?>"  
