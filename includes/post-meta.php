...
Container::make('post_meta', 'Additional Documents')
    ->set_priority('high')
    ->where('post_type', '=', 'qualifications')
    ->add_fields(
        array(
            Field::make('complex', 'additional_documents', '')
                ->add_fields(
                    array(
                        Field::make('text', 'document_title', __('Document Title'))->set_width(50),
                        Field::make('file', 'document_file', __('Document File'))->set_width(50)
                            ->set_type('application/pdf'),

                    )
                )
                ->set_header_template('<%- document_title %>')
                ->set_layout('tabbed-vertical')
        )
    );*/
...
Container::make('theme_options', 'Blog Settings')
    ...
    ->add_fields(
        array(
            ...
            Field::make('image', 'post_page_banner', __('Blog Page Hero')),
        )
    );
